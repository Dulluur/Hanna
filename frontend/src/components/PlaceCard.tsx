...
export function PlaceCard({place, upsell}: Props){
  ...
  return(
    ...
    >
      {place.photo_url ? (
        <img src={place.photo_url} alt={place.name} className='aspect-[16/9] w-full object-cover' loading='lazy'/>
      ): (
        <ImagePlaceholder
          seed={place.name}
          alt={place.name}
      ...
      <CardContent className='p-4 pt-3'>
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h3 className="truncate text-base font-semibold leading-tight">{place.name}
        ...
        {upsell &&(
          <div className="mt-3 rounded-md bg-primary/10 p-2.5">
            ...
            <ul className="mt-1.5 space-y-0.5 text-xs text-muted-foreground">{upsell.reasons.map((r,i) =>(
              <li key={i}>· {r}</li>
            ))}
            </ul>
          </div>
