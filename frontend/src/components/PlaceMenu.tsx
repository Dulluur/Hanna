...
const PRICE_BUCKETS: {code: string; label: string; min:number; max:number | null} []=[
...
export function PlaceMenu({dishes}: Props){
  ...
  const filtered = useMemo(() =>{
    const range = PRICE_BUCKETS.find((b) => b.code === bucket) ?? PRICE_BUCKETS[0]!
  ...
  return(
    <div className="space-y-3">
      <div role='radiogroup' aria-label='Фильтр по цене' className='flex flex-wrap gap-1.5'>
        {PRICE_BUCKETS.map((b) => {
      ...
      {dish && (
        <div className='space-y-3'>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {dish.weight && (
              <span className='text-sm text-muted-foreground'>{dish.weight}</span>
            )}
          </div>
          ...
          {dish.description && (
            <p className="whitespace-pre-line text-sm leading-relaxed">{dish.description}</p>
          )}
